...
class SpaceView(GeneralToolClass):
            ...
            if self.nBody == 'all':
                for k in range(self.InputData['Planets']['Nb']):
                    ...
                    if self.CheckDateObs.CheckParam.isChecked():
                        self.annotate_dates(dates, ra, dec)
    ...
    def annotate_dates(self, dates, ra, dec):
        """Annotate dates on the plot."""
        ra_range = abs(max(ra) - min(ra))
        dec_range = abs(max(dec) - min(dec))
        ...
        for idx in range(len(dates)):
            ...
            if not overlap:
                self.Subplot2D.annotate(f"{dates[idx]:.0f}", (x_right, y), color='black', fontsize=8, ha='left')
                annotated_points.append((x_right, y))
            else:
                annotated_points.append((x_left, y))
                if not overlap:
                    self.Subplot2D.annotate(f"{dates[idx]:.0f}", (x_left, y), color='black', fontsize=8, ha='right')
